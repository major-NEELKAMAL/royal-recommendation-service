...
public class RecommendationService {
  ...
  /**
   * @param postTypeInt Pass the smallint numeric identifier (e.g. Image value vs Video value)
   */
  public List<Long> getRecommendationsForUser(int postTypeInt, long userId, int offset, int limit) {
    ...
    Dataset<Row> rawUserDF =
        spark.createDataFrame(Collections.singletonList(new UserId(userId)), UserId.class)
    ...
    String sql = String.format("SELECT CAST(post_id_str AS LONG) as post_id FROM user_recs "
        + "WHERE post_type = %d " + "ORDER BY score DESC LIMIT %d OFFSET %d", postTypeInt, limit,
        offset);
